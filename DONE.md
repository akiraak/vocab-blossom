...
- 2026-08-16 出題順がABC順になっているのでランダムにする（単語 ID と端末ごとの種から並びを決める `ShuffleOrder` を追加。新規学習プールはレベル内をまるごとシャッフル、復習は期限順に選んで出題順はシャッフル。同じ端末なら並びは毎回同じなのでホームの表示・中断再開とずれない）→ [plan](docs/plans/archive/random-question-order.md)
